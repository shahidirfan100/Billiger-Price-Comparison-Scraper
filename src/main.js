    ...
    try {
        ...
        function extractOffersFromHtml($, baseUrl) {
            ...
            $('div[class*="offer"], [data-offer-row], [class*="price-row"]').each((_, el) => {
                ...
                if (shopBtn.length) shopName = shopBtn.attr('aria-label') || shopBtn.attr('title') || '';
                if (!shopName) shopName = $el.find('img[alt]').first().attr('alt') || '';
                if (!shopName) shopName = $el.find('a[title]').first().attr('title') || '';
                ...
                if (redirectLink) offerUrl = toAbs(redirectLink, baseUrl);
                else {
                    const anyLink = $el.find('a[href^="http"]').attr('href');
                    if (anyLink) offerUrl = anyLink;
                }

                offers.push({ shop_name: shopName, price, shipping_cost: shipping, total_price: total, offer_url: offerUrl });
            });
        ...
        function extractProductFromHtml($) {
            ...
            const brand = $('meta[itemprop="brand"]').attr('content') || $('[class*="brand"]').first().text().trim() || null;

            let lowestPrice = cleanPrice($('meta[itemprop="lowPrice"]').attr('content')) || cleanPrice($('[class*="price"]').first().text());
            let highestPrice = cleanPrice($('meta[itemprop="highPrice"]').attr('content'));

            let offerCount = parseInt($('meta[itemprop="offerCount"]').attr('content'), 10) || null;
            if (!offerCount) {
                const offerMatch = $('[href="#offers"], [class*="offer-count"]').text().match(/(\d+)\s*Angebote?/i);
                if (offerMatch) offerCount = parseInt(offerMatch[1], 10);
            }

            const imageUrl = $('meta[property="og:image"]').attr('content') || $('img[class*="product"]').attr('src') || null;
            const rating = parseFloat($('meta[itemprop="ratingValue"]').attr('content')) || null;
            const reviewCount = parseInt($('meta[itemprop="reviewCount"]').attr('content'), 10) || null;

            return {
                product_name: title || null, brand, gtin: null, sku: null,
                lowest_price: lowestPrice, highest_price: highestPrice, offer_count: offerCount,
                rating, review_count: reviewCount, image_url: imageUrl, currency: 'EUR',
        ...
        if (!initial.length && searchQuery) initial.push(buildSearchUrl(searchQuery));
        if (!initial.length) initial.push(buildSearchUrl('laptop'));
        ...
        const crawler = new CheerioCrawler({
            ...
            async requestHandler({ request, $, enqueueLinks }) {
                ...
                if (urlType === 'SEARCH') {
                    ...
                    for (const link of toEnqueue) seenUrls.add(link);
                ...
                if (urlType === 'PRICELIST' || urlType === 'BASEPRODUCT' || urlType === 'PRODUCT' || request.userData?.label === 'DETAIL') {
                    ...
                    try {
                        ...
                        if (!productData || !productData.product_name) {
                            productData = extractProductFromHtml($);
                        }

                        if (productData) {
                            const htmlData = extractProductFromHtml($);
                            if (!productData.lowest_price && htmlData.lowest_price) productData.lowest_price = htmlData.lowest_price;
                            if (!productData.highest_price && htmlData.highest_price) productData.highest_price = htmlData.highest_price;
                            if (!productData.offer_count && htmlData.offer_count) productData.offer_count = htmlData.offer_count;
                            if (!productData.image_url && htmlData.image_url) productData.image_url = htmlData.image_url;
                        }

                        if (!productData || !productData.product_name) return;

                        const offers = collectOffers ? extractOffersFromHtml($, request.url) : [];
                        ...
                        if (batch.length >= BATCH_SIZE) {
                            await flushBatch();
                            log.info(`Saved ${saved}/${RESULTS_WANTED} products`);
                        }
                    } catch { /* skip failed */ }
                }
            },
        ...
        log.info(`Starting scraper for ${RESULTS_WANTED} products`);
        await crawler.run(initial.map(u => ({ url: u, userData: { label: getUrlType(u) === 'SEARCH' ? 'SEARCH' : 'DETAIL', pageNo: 1 } })));
